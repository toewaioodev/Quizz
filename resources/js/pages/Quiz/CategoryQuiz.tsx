...
export default function CategoryQuiz({ categoryId, categoryName }: { categoryId: string; categoryName: string }) {
    ...
    return (
        <div className="min-h-screen bg-slate-50 font-sans text-slate-900 transition-colors duration-300 dark:bg-slate-950 dark:text-gray-100">
            ...
            {/* Header */}
            <header className="fixed top-0 left-0 right-0 z-50 border-b border-slate-200 bg-white/80 px-4 py-3 backdrop-blur-md dark:border-slate-800 dark:bg-slate-900/80">
            ...
            {/* Progress Bar */}
            <div className="fixed top-[61px] left-0 right-0 z-40 h-1 bg-slate-200 dark:bg-slate-800">
                <motion.div
                    initial={{ width: 0 }}
                    ...
                    <AnimatePresence mode="wait">
                        {loading ? (
                            ...
                            >
                                ...
                                <p className="mt-4 font-medium text-slate-500">{t('Loading Question...')}</p>
                            </motion.div>
                        ) : currentQuestion ? (
                            <motion.div
                            ...
                            >
                                ...
                                <div className="relative overflow-hidden rounded-3xl bg-white p-8 shadow-2xl dark:bg-slate-800/50 dark:ring-1 dark:ring-white/10">
                                    <h2 className="text-center text-2xl font-black leading-tight text-slate-800 md:text-3xl dark:text-white">
                                        ...
                                        return (
                                            ...
                                            >
                                                <div className="flex items-center gap-3">
                                                    <div className={`flex h-8 w-8 shrink-0 items-center justify-center rounded-lg border text-sm font-bold transition-colors ${result && (opt === result.correct_answer || (opt === result.selected_option && !result.correct))
                                ...
                                <AnimatePresence>
                                    {result && (
                                        ...
                                        >
                                            <div className="mx-auto flex max-w-4xl items-center justify-between">
                                                <div className="flex items-center gap-4">
                                                    <div className={`flex h-12 w-12 items-center justify-center rounded-full text-2xl ${result.correct ? 'bg-green-100 text-green-600' : 'bg-red-100 text-red-600'}`}>
                                                        {result.correct ? '✓' : '✗'}
                                                    </div>
                                                    <div>
                            ...
                            <div className="text-center text-red-500">
                                ...
                                <button onClick={() => fetchQuestion()} className="mt-4 underline">{t('Try Again')}</button>
                            </div>
                        )}
                    </AnimatePresence>
                </div>
