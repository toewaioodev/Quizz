...
function LobbyContent() {
  ...
  const handleInvite = useCallback((inviteeId) => {
  ...
  return /* @__PURE__ */ jsxs("div", { className: "relative flex min-h-screen flex-col items-center overflow-hidden bg-slate-50 p-0 text-slate-900 transition-colors duration-300 dark:bg-slate-950 dark:text-white", children: [
    ...
    /* @__PURE__ */ jsxs("div", { className: "z-10 mt-8 grid w-full max-w-4xl grid-cols-1 items-start gap-8 p-6 md:grid-cols-2", children: [
      ...
      /* @__PURE__ */ jsx(ActivePlayersList, { activeUsers, user, handleInvite })
    ] })
  ] });
}
const LobbyStats = memo(({ user, searching, findMatch }) => {
  const { t } = useTranslation();
  return /* @__PURE__ */ jsxs("div", { className: "rounded-2xl border border-slate-200 bg-white/50 p-6 shadow-xl backdrop-blur-xl dark:border-white/10 dark:bg-slate-900/50", children: [
...
const ActivePlayersList = memo(({ activeUsers, user, handleInvite }) => {
  const { t } = useTranslation();
  return /* @__PURE__ */ jsxs("div", { className: "h-full min-h-[400px] rounded-2xl border border-slate-200 bg-white/30 p-6 backdrop-blur-md dark:border-white/5 dark:bg-slate-900/30", children: [
          ...
          children: [
            ...
            /* @__PURE__ */ jsxs("div", { className: "flex-1", children: [
              /* @__PURE__ */ jsxs("p", { className: "text-sm font-medium text-slate-800 dark:text-slate-200", children: [
                p.name,
                " ",
                p.id == user.id && /* @__PURE__ */ jsxs("span", { className: "text-green-600 dark:text-green-400", children: [
                  "(",
                  t("You"),
                  ")"
                ] })
            ...
            p.status === "searching" && /* @__PURE__ */ jsxs("div", { className: "flex items-center gap-1 rounded-full bg-blue-100 px-2 py-0.5 text-xs font-medium text-blue-600 dark:bg-blue-900/30 dark:text-blue-400", children: [
              /* @__PURE__ */ jsx("div", { className: "h-1.5 w-1.5 animate-pulse rounded-full bg-blue-500" }),
              t("Finding")
            ] }),
            p.status === "playing" && /* @__PURE__ */ jsx("div", { className: "flex items-center gap-1 rounded-full bg-orange-100 px-2 py-0.5 text-xs font-medium text-orange-600 dark:bg-orange-900/30 dark:text-orange-400", children: t("Playing") }),
            p.id != user.id && p.status !== "playing" && /* @__PURE__ */ jsx(
              "button",
              {
                onClick: () => handleInvite(p.id),
                className: "ml-2 rounded-lg bg-indigo-100 px-3 py-1 text-xs font-medium text-indigo-700 transition-colors hover:bg-indigo-200 dark:bg-indigo-500/20 dark:text-indigo-300 dark:hover:bg-indigo-500/30",
                children: t("Invite")
              }
            )
          ]
